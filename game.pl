...
generate_move(Board, Frog, Move) :-
    ...
    !, generate_jumps(Board, Frog, FrogList, PossibleMoves), !,
...
/**
 * Get jumps
 * get_jumps(+Board, +Frog, +Pos, -JumpPositionsList)
 * Generates all the valid jumps from a start position
 *
 * Board -> Initial Board
 * Frog -> Player Frog
...
get_jumps(Board, Frog, Pos, JumpList) :-
    write('get_jumps'), nl, 
    write('get_jumps Pos '), print_position(Pos), nl, wait_for_input,
    !,
    (
        bagof(EndPos, frog_can_jump(Board, Pos, EndPos), PositionList), !;
        PositionList = []
    ),
    write('get_jumps PositionList '),
    print_list(PositionList), nl, wait_for_input,
    ...
    %write('get_jumps JumpList '), print_list(JumpList), nl, wait_for_input,
    !. %append de cada lista de NewJumpList ao valor de Pos

/**
 * Call new get jumps
 * call_new_get_jumps(+Board, +Frog, +StartPosition, +OtherPositions, -JumpList)
 * Calls get_jumps for all the new positions 
 * 
 * Board -> Initial Board
 * Frog -> Player Frog
 * StartPosition -> Start Position of the current jump
 * OtherPositions -> Other positions generated by the bagof call of get_jumps
 * JumpList -> List of lists with all the jumps from the start position
 */
call_new_get_jumps(_, _, _, [], []).

call_new_get_jumps(Board, Frog, StartPos, [EndPos|OtherPos], JumpList) :-
    write('get_mid_pos'), nl, wait_for_input,
    middle_position(StartPos, EndPos, MidPos), %Get middle position
    write('jump'), nl, wait_for_input,
    jump(Board, StartPos, MidPos, EndPos, Frog, NewBoard), %Get the board were each jump is executed
    !,write('call_get_jumps'), print_position(EndPos), nl, wait_for_input, 
    get_jumps(NewBoard, Frog, EndPos, NewList), 
    append([EndPos], NewList, ThisJumpList),
    !, call_new_get_jumps(Board, Frog, StartPos, OtherPos, NewJumpList), !,
    append(ThisJumpList, NewJumpList, JumpList). 


/**
 * Execute Move
 * execute_move(+InputBoard, +Frog, +PositionsList, -OutputBoard)
 * Executes a cpu move
 * 
